...
from helpers import helpers_litter as hp, helpersWindows as hw
import numpy as np
import cv2
...
class LitterDetectionApp:
    def __init__(self,master, mode="Light"):
        self.master = master
        self.window = tk.Tk()
        self.window.title("Litter Detection")
        ...
        self.image_on_canvas = None  # Add this line
    ...
    def create_widgets(self):
        # Title
        self.title = tk.Label(self.frame, text="Litter Detection",
                              font=("Arial", 16, "bold"))
        ...
        self.buton_detect = self.create_button("Detect Litter", self.detectLitter, relx=0.675, rely=0.20)
        ...
        self.buton_detectie_videoclip = self.create_button("Detect Litter in Video", self.detect_video, relx=0.675, rely=0.75)
        self.buton_preview_detectie = self.create_button("Watch Last Detection", self.preview_detection, relx=0.675, rely=0.85)
    ...
    def imageUpload(self):
        ...
        if self.cale_poza:
            ...
            self.pozaCanvas.delete("all")

            # Create new image on canvas
            self.image_on_canvas = self.pozaCanvas.create_image(0, 0, anchor=tk.NW, image=self.test_photo)
            self.window.update_idletasks()

    def detectLitter(self):
    ...
    def show_image(self):
        if self.detect_photo:
            ...
            messagebox.showerror("Error", "No detected image available.")
    ...
    def preview_detection(self):
        self.play_video("../Results/results_litter.mp4")
    ...
    def save_image_pil(self, image_pil):
        ...
        if save_path:
            try:
                ...
                messagebox.showinfo("Save", f"Image has been saved to: {save_path}")
            except Exception as e:
                messagebox.showerror("Error", f"Saving image failed: {e}")

    def play_video(self, video_path):
        import os

        if not os.path.isfile(video_path):
            messagebox.showerror("Error", f"The video '{video_path}' does not exist.")
            return
...
# # Usage
# app = LitterDetectionApp(mode="Dark")  # or "Dark"
# app.run()